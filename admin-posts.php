...
<?php
    if (isset($_POST["publish_post"])){
        ...
        elseif (strlen($title) < 6){
            $_SESSION["ErrorMessage"] = "Post Title should be greater than 6 characters!";
        }
        elseif (empty($description)){
            $_SESSION["ErrorMessage"] = "Description can't be empty!";
        }
        else{
            $query = "insert into posts(date_time, post_title, post_image, post_description) values (?, ?, ?, ?)";
            $result = mysqli_prepare($connectingDB, $query);
            move_uploaded_file($_FILES["image"]["tmp_name"], $target);

            if ($result){
                mysqli_stmt_bind_param($result, "ssss", $dateTime, $title, $image, $description);
...
<head>
    ...
    <title>Cryjaxx Admin | Posts </title>
                ...
                <div class="col-lg-8">
                    <article class="blog_item" style="padding: 20px; background-color: #f6faff">
                        <div class="blog-details">
                            <form role="form" action="admin-posts.php" method="post" enctype="multipart/form-data">
                                <h3 style="font-weight: bold">Add New <span style="color: #27aae1">Post</span></h3>
                                ...
                                <button class="button rounded-0 primary-bg text-white w-100 btn_1 boxed-btn" type="submit" name="publish_post">Publish</button>
                            </form>
                        </div>
                    </article>
                </div>
            ...
            <div class="row">
                <div class="col-lg-8 posts-list">
                    <div class="single-post">
                        <h2 style="font-weight: bold">All <span style="color: #27aae1">Cryjaxx</span> posts</h2>
                        <p><i class="fas fa-exclamation-circle"></i> <strong style="color: #27aae1">Note: </strong><br>Here are all posts of Cryjaxx. <br><strong>Administrator</strong> can see all posts and has the power to delete a post if it's needed for privacy or content reasons!</p>
                    </div>
                </div>
                <div class="col-lg-4">
                    ...
                    <div class="row razo-blog-masonary">
                        <?php
                            ...
                            if(isset($_GET["SearchButton"])){
                                $Search = $_GET["Search"];
                                $sql = "SELECT * FROM posts WHERE post_title LIKE '%{$Search}%' OR post_description LIKE '%{$Search}%'; ";
                                $result = mysqli_query($connectingDB, $sql);
                            }
                            else{
                                        ...
                                        <div class="post-content">
                                            ...
                                            <div class="row">
                                                <div class="col-12 text-center">
                                                    <a href="deletePost.php?id=<?php echo $id;?>"><button class="button rounded-0 primary-bg text-white w-100 btn_1 boxed-btn1" type="submit">Delete</button></a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
